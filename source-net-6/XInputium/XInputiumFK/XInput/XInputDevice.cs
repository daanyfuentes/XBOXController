...
public class XInputDevice : InputDevice<XInputDeviceState>
{
    ...
    private static readonly XInputUserIndex[] s_UserIndexes = new XInputUserIndex[4] { XInputUserIndex.One, XInputUserIndex.Two, XInputUserIndex.Three, XInputUserIndex.Four};
    ...
    public XInputDevice(XInputUserIndex userIndex)
        : base()
    {
        if (!Enum.IsDefined(typeof(XInputUserIndex), userIndex))
        ...
        float leftMotorSpeed, float rightMotorSpeed)
    {
        ...
        leftMotorSpeed = leftMotorSpeed.Clamp( 0, 1);
        rightMotorSpeed = rightMotorSpeed.Clamp( 0, 1);

        // Set the controller motors speed.
        XINPUT_VIBRATION vibration = new()
        {
            wLeftMotorSpeed = (ushort)(leftMotorSpeed * ushort.MaxValue).Clamp( ushort.MinValue, ushort.MaxValue),
            wRightMotorSpeed = (ushort)(rightMotorSpeed * ushort.MaxValue).Clamp( ushort.MinValue, ushort.MaxValue),
        };
