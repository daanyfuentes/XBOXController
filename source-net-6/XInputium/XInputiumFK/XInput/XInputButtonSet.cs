    ...
    IReadOnlyCollection<XInputButton>
{
    ...
    public int PressedButtonsCount
    {
        ...
        private set
        {
            if (SetProperty(ref _pressedButtonsCount,
                value.Clamp( 0, _buttonAssociations.Count),
    ...
    private XInputButton CreateAndRegisterButton(XButtons button)
    {
        ...
        if (!_buttonAssociations.ContainsKey(button))
        {
            ...
            _buttonAssociations.Add(button,
            new ButtonAssociation(xInputButton, updateCallback, new DigitalButtonEventArgs<XInputButton>(xInputButton)));
    ...
    {
        if (button == XButtons.None || !Enum.IsDefined(typeof(XButtons), button))
            throw new ArgumentException(
