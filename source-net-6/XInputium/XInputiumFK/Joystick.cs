...
public class Joystick : EventDispatcherObject
{
        ...
        protected override JoystickSample GetAverage(int samplesCount, JoystickSample totalSum)
        {
            if (samplesCount > 0)
            {
                JoystickSample average = new(totalSum.X / samplesCount,
                    totalSum.Y / samplesCount, new TimeSpan(totalSum.Time.Ticks / samplesCount));
                return average;
            }
            else
            {
    ...
    public float MovementSpeed
    {
        ...
        private set
        {
            SetProperty(ref _movementSpeed, Math.Max(value, 0f), s_EA_MovementSpeed);
        }
    }
        ...
        float normalAngle)
    {
        ...
        normalAngle = Math.Abs(normalAngle);
        normalAngle -= (float)Math.Floor(normalAngle);
    ...
    private void RegisterSmoothingSample(in float x, in float y, in TimeSpan time)
    {
        if (SmoothingFactor > 0f && SmoothingSamplePeriod > TimeSpan.Zero)
        {
                ...
                && _smoothingSamples.Average.Time > TimeSpan.Zero)
            {
                ...
                sampleLength = sampleLength.Clamp(MinSmoothingSamples, MaxSmoothingSamples);
    ...
    {
        if (!Enum.IsDefined(typeof(JoystickDirection), direction))
