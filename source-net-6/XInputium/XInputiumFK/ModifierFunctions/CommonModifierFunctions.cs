...
public static class CommonModifierFunctions
{
    ...
    public static ModifierFunction Reverse { get; }
        = value => (float)((double)(1f - Math.Min(Math.Abs(value), 1f))).CopySign(value);
        ...
        float innerDeadZone, float outerDeadZone)
    {
        ...
        return value => (float)((double)Math.Abs(InputMath.ApplyDeadZone(value, innerDeadZone, outerDeadZone))).CopySign(value);
    }
    ...
    public static ModifierFunction Quantize(float stepSize)
    {
        ...
        if (stepSize == 0f)
            ...
            return value => (float)Math.Floor(value / stepSize) * stepSize;
    }
